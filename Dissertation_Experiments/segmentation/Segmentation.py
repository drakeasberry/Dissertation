...
text_lang_hist_qnum = visual.TextStim(win=win, name='text_lang_hist_qnum',
    ...
    languageStyle='LTR',
    depth=-3.0);
rating_lang_hist = visual.RatingScale(win=win, name='rating_lang_hist', marker='triangle', size=1.5, pos=[0.0, -0.4], low=0, high=20, labels=['0', ' 20+'], scale='0 = As long as I can remember ... 20+ = Not yet comfortable', markerStart='10')
...
text_lang_hist_qnum_2 = visual.TextStim(win=win, name='text_lang_hist_qnum_2',
    ...
    languageStyle='LTR',
    depth=-3.0);
rating_lang_hist_2 = visual.RatingScale(win=win, name='rating_lang_hist_2', marker='triangle', size=1.5, pos=[0.0, -0.4], low=0, high=20, labels=['0', ' 20+'], scale='0 = Never     ...      20+ = More than 20 years', markerStart='10')
...
text_lang_use_qnum = visual.TextStim(win=win, name='text_lang_use_qnum',
    ...
    languageStyle='LTR',
    depth=-3.0);
rating_blp_use = visual.RatingScale(win=win, name='rating_blp_use', scale = None,
...
depth = 0,
autoLog = True,
#scaleDescription = 'Each question must sum to 100%')
...
text_lang_prof_qnum = visual.TextStim(win=win, name='text_lang_prof_qnum',
    ...
    languageStyle='LTR',
    depth=-3.0);
rating_lang_prof = visual.RatingScale(win=win, name='rating_lang_prof', marker='triangle', size=1.5, pos=[0.0, -0.4], choices=['0', '1', '2', '3', '4', '5', '6'], tickHeight=-1, markerStart='3')
...
text_lang_att_qnum = visual.TextStim(win=win, name='text_lang_att_qnum',
    ...
    languageStyle='LTR',
    depth=-3.0);
rating_lang_att = visual.RatingScale(win=win, name='rating_lang_att', marker='triangle', size=1.5, pos=[0.0, -0.4], low=0, high=6, labels=['0', '6'], scale='0 = disagree ... 6 = agree', markerStart='3')
...
trials_Debug = data.TrialHandler(nReps=0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
    seed=None, name='trials_Debug')
thisExp.addLoop(trials_Debug)  # add the loop to the experiment
thisTrials_Debug = trials_Debug.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisTrials_Debug.rgb)
if thisTrials_Debug != None:
    for paramName in thisTrials_Debug:
        exec('{} = thisTrials_Debug[paramName]'.format(paramName))

for thisTrials_Debug in trials_Debug:
    currentLoop = trials_Debug
    # abbreviate parameter names if possible (e.g. rgb = thisTrials_Debug.rgb)
    if thisTrials_Debug != None:
        for paramName in thisTrials_Debug:
            exec('{} = thisTrials_Debug[paramName]'.format(paramName))
...
# completed 0 repeats of 'trials_Debug'
...
trials_blp_hist = data.TrialHandler(nReps=1, method='sequential', 
    extraInfo=expInfo, originPath=-1,
    trialList=data.importConditions('data/processed_data/exp_files/sp_en_blp_trials.csv', selection='0:3'),
