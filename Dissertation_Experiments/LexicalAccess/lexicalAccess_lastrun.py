...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/drakeasberry/Desktop/LexicalAccess/lexicalAccess_lastrun.py',
...
ciclo = data.TrialHandler(nReps=1, method='random', 
...
for thisCiclo in ciclo:
    ...
    # keep track of which components have finished
    pruebasComponents = [text_Word, text_Si, text_No, lexEsp_key_resp, text_Mask, text_Prime]
    ...
    while continueRoutine:
        ...
        # *text_Word* updates
        if frameN >= 61 and text_Word.status == NOT_STARTED:
            # keep track of start time/frame for later
            text_Word.tStart = t
        ...
        # *text_Si* updates
        if frameN >= 61 and text_Si.status == NOT_STARTED:
            # keep track of start time/frame for later
            text_Si.tStart = t
        ...
        # *text_No* updates
        if frameN >= 61 and text_No.status == NOT_STARTED:
            # keep track of start time/frame for later
            text_No.tStart = t
        ...
        if frameN >= 61 and lexEsp_key_resp.status == NOT_STARTED:
            # keep track of start time/frame for later
        ...
        if lexEsp_key_resp.status == STARTED:
            ...
            if len(theseKeys) > 0:  # at least one key was pressed
                if lexEsp_key_resp.keys == []:  # then this was the first keypress
                    ...
                    # was this 'correct'?
                    if (lexEsp_key_resp.keys == str(corrAnsLexical)) or (lexEsp_key_resp.keys == corrAnsLexical):
        ...
        # *text_Mask* updates
        if frameN >= 0 and text_Mask.status == NOT_STARTED:
            # keep track of start time/frame for later
            text_Mask.tStart = t
            text_Mask.frameNStart = frameN  # exact frame index
            text_Mask.setAutoDraw(True)
        if text_Mask.status == STARTED and frameN >= (text_Mask.frameNStart + 30):
            text_Mask.setAutoDraw(False)
        
        # *text_Prime* updates
        if frameN >= 31 and text_Prime.status == NOT_STARTED:
            # keep track of start time/frame for later
            text_Prime.tStart = t
            text_Prime.frameNStart = frameN  # exact frame index
            text_Prime.setAutoDraw(True)
        if text_Prime.status == STARTED and frameN >= (text_Prime.frameNStart + 30):
