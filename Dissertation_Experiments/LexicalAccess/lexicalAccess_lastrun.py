...
expName = 'LexicalAccess'  # from the Builder filename that created this script
expInfo = {'01_Participante (Participant):': 'part000', '02_Sesión (Session):': ['A', 'C'], '03_Nombre (First name):': '', '04_Apellido (Last name):': '', '05_Edad (Age):': '', '06_Sexo (Gender):': ['', 'Hombre', 'Mujer'], '07_País de nacimiento (Birth country):': ['', 'México', 'Estados Unidos'], '08_Lugar de residencia (Place of residence):': ['', 'Tucson', 'Hermosillo'], '09_Nivel más alto de formación académica (Highest level of formal education):': ['', 'Menos de la escuela secundaria', 'Escuela Secundaria', 'Un poco de universidad', 'Universidad (diplomatura, licenciatura)', 'Un poco de escuela graduada', 'Máster', 'Doctorado'], '10_Idioma preferido (Preferred language):': ['', 'español', 'inglés']}
...
thisExp = data.ExperimentHandler(name=expName, version='',
    ...
    originPath='/Users/drakeasberry/github/Dissertation/Dissertation_Experiments/lexicalAccess/lexicalAccess_lastrun.py',
...
pracLexInstr = visual.TextStim(win=win, name='pracLexInstr',
...
# Initialize components for Routine "lexDesPrac"
lexDesPracClock = core.Clock()
...
text_Si = visual.TextStim(win=win, name='text_Si',
    text='sí',
    font='Arial',
    pos=[-0.5, -.5], height=0.2, wrapWidth=None, ori=0, 
    color='green', colorSpace='rgb', opacity=1, 
    languageStyle='LTR',
    depth=-1.0);
...
text_Mask = visual.TextStim(win=win, name='text_Mask',
    ...
    languageStyle='LTR',
    depth=-4.0);
text_Prime = visual.TextStim(win=win, name='text_Prime',
    text='default text',
    font='Arial',
    pos=(0, 0), height=0.2, wrapWidth=None, ori=0, 
    color='white', colorSpace='rgb', opacity=1, 
...
trials_debug_1 = data.TrialHandler(nReps=0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
    seed=None, name='trials_debug_1')
thisExp.addLoop(trials_debug_1)  # add the loop to the experiment
thisTrials_debug_1 = trials_debug_1.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisTrials_debug_1.rgb)
if thisTrials_debug_1 != None:
    for paramName in thisTrials_debug_1:
        exec('{} = thisTrials_debug_1[paramName]'.format(paramName))

for thisTrials_debug_1 in trials_debug_1:
    currentLoop = trials_debug_1
    # abbreviate parameter names if possible (e.g. rgb = thisTrials_debug_1.rgb)
    if thisTrials_debug_1 != None:
        for paramName in thisTrials_debug_1:
            exec('{} = thisTrials_debug_1[paramName]'.format(paramName))
...
# completed 0 repeats of 'trials_debug_1'


# set up handler to look after randomisation of conditions etc
trials_debug_2 = data.TrialHandler(nReps=1, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
    seed=None, name='trials_debug_2')
thisExp.addLoop(trials_debug_2)  # add the loop to the experiment
thisTrials_debug_2 = trials_debug_2.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisTrials_debug_2.rgb)
if thisTrials_debug_2 != None:
    for paramName in thisTrials_debug_2:
        exec('{} = thisTrials_debug_2[paramName]'.format(paramName))

for thisTrials_debug_2 in trials_debug_2:
    currentLoop = trials_debug_2
    # abbreviate parameter names if possible (e.g. rgb = thisTrials_debug_2.rgb)
    if thisTrials_debug_2 != None:
        for paramName in thisTrials_debug_2:
            exec('{} = thisTrials_debug_2[paramName]'.format(paramName))
    ...
    continueRoutine = True
    # update component parameters for each repeat
    pracLex_Start = event.BuilderKeyResponse()
    # keep track of which components have finished
    pracInsComponents = [pracLexInstr, pracLex_Start]
    ...
    while continueRoutine:
        ...
        # *pracLexInstr* updates
        if t >= 0.0 and pracLexInstr.status == NOT_STARTED:
            # keep track of start time/frame for later
            pracLexInstr.tStart = t
            pracLexInstr.frameNStart = frameN  # exact frame index
            pracLexInstr.setAutoDraw(True)
        
        # *pracLex_Start* updates
        if t >= 0.0 and pracLex_Start.status == NOT_STARTED:
            # keep track of start time/frame for later
            pracLex_Start.tStart = t
            pracLex_Start.frameNStart = frameN  # exact frame index
            pracLex_Start.status = STARTED
            # keyboard checking is just starting
            event.clearEvents(eventType='keyboard')
        if pracLex_Start.status == STARTED:
            theseKeys = event.getKeys(keyList=['0'])
    ...
    pracTrialLoop = data.TrialHandler(nReps=1, method='random', 
        extraInfo=expInfo, originPath=-1,
        trialList=data.importConditions('LexicalAccess.csv'),
    ...
    for thisPracTrialLoop in pracTrialLoop:
        ...
        # ------Prepare to start Routine "lexDesPrac"-------
        t = 0
        lexDesPracClock.reset()  # clock
        frameN = -1
        continueRoutine = True
        # update component parameters for each repeat
        text_Word.setText(Word)
        lexEsp_key_resp_2 = event.BuilderKeyResponse()
        text_Mask.setText(Mask)
        text_Prime.setText(Prime)
        # keep track of which components have finished
        lexDesPracComponents = [text_Word, text_Si, text_No, lexEsp_key_resp_2, text_Mask, text_Prime]
        for thisComponent in lexDesPracComponents:
            if hasattr(thisComponent, 'status'):
                thisComponent.status = NOT_STARTED
        
        # -------Start Routine "lexDesPrac"-------
        while continueRoutine:
            # get current time
            t = lexDesPracClock.getTime()
            frameN = frameN + 1  # number of completed frames (so 0 is the first frame)
            # update/draw components on each frame
            
            # *text_Word* updates
            if t >= 1.12 and text_Word.status == NOT_STARTED:
                # keep track of start time/frame for later
                text_Word.tStart = t
            ...
            # *text_Si* updates
            if t >= 1.12 and text_Si.status == NOT_STARTED:
                # keep track of start time/frame for later
                text_Si.tStart = t
            ...
            # *text_No* updates
            if t >= 1.12 and text_No.status == NOT_STARTED:
                # keep track of start time/frame for later
                text_No.tStart = t
                text_No.frameNStart = frameN  # exact frame index
                text_No.setAutoDraw(True)
            
            # *lexEsp_key_resp_2* updates
            if t >= 1.12 and lexEsp_key_resp_2.status == NOT_STARTED:
                # keep track of start time/frame for later
                lexEsp_key_resp_2.tStart = t
                lexEsp_key_resp_2.frameNStart = frameN  # exact frame index
                lexEsp_key_resp_2.status = STARTED
                # keyboard checking is just starting
                win.callOnFlip(lexEsp_key_resp_2.clock.reset)  # t=0 on next screen flip
                event.clearEvents(eventType='keyboard')
            if lexEsp_key_resp_2.status == STARTED:
                theseKeys = event.getKeys(keyList=['1', '4'])
                ...
                if "escape" in theseKeys:
                    endExpNow = True
                if len(theseKeys) > 0:  # at least one key was pressed
                    if lexEsp_key_resp_2.keys == []:  # then this was the first keypress
                        lexEsp_key_resp_2.keys = theseKeys[0]  # just the first key pressed
                        lexEsp_key_resp_2.rt = lexEsp_key_resp_2.clock.getTime()
                        # was this 'correct'?
                        if (lexEsp_key_resp_2.keys == str(corrAnsLexical)) or (lexEsp_key_resp_2.keys == corrAnsLexical):
                            lexEsp_key_resp_2.corr = 1
                        else:
                            lexEsp_key_resp_2.corr = 0
                        # a response ends the routine
                        continueRoutine = False
            
            # *text_Mask* updates
            if t >= 0 and text_Mask.status == NOT_STARTED:
                # keep track of start time/frame for later
                text_Mask.tStart = t
                text_Mask.frameNStart = frameN  # exact frame index
                text_Mask.setAutoDraw(True)
            frameRemains = 1 - win.monitorFramePeriod * 0.75  # most of one frame period left
            if text_Mask.status == STARTED and t >= frameRemains:
                text_Mask.setAutoDraw(False)
            
            # *text_Prime* updates
            if t >= 1 and text_Prime.status == NOT_STARTED:
                # keep track of start time/frame for later
                text_Prime.tStart = t
                text_Prime.frameNStart = frameN  # exact frame index
                text_Prime.setAutoDraw(True)
            frameRemains = 1.12 - win.monitorFramePeriod * 0.75  # most of one frame period left
            if text_Prime.status == STARTED and t >= frameRemains:
                text_Prime.setAutoDraw(False)
        ...
        # -------Ending Routine "lexDesPrac"-------
        for thisComponent in lexDesPracComponents:
            if hasattr(thisComponent, "setAutoDraw"):
                thisComponent.setAutoDraw(False)
        # check responses
        if lexEsp_key_resp_2.keys in ['', [], None]:  # No response was made
            lexEsp_key_resp_2.keys=None
            # was no response the correct answer?!
            if str(corrAnsLexical).lower() == 'none':
               lexEsp_key_resp_2.corr = 1;  # correct non-response
            else:
               lexEsp_key_resp_2.corr = 0;  # failed to respond (incorrectly)
        # store data for pracTrialLoop (TrialHandler)
        pracTrialLoop.addData('lexEsp_key_resp_2.keys',lexEsp_key_resp_2.keys)
        pracTrialLoop.addData('lexEsp_key_resp_2.corr', lexEsp_key_resp_2.corr)
        if lexEsp_key_resp_2.keys != None:  # we had a response
            pracTrialLoop.addData('lexEsp_key_resp_2.rt', lexEsp_key_resp_2.rt)
        # the Routine "lexDesPrac" was not non-slip safe, so reset the non-slip timer
        routineTimer.reset()
...
# completed 1 repeats of 'trials_debug_2'


# set up handler to look after randomisation of conditions etc
trials_debug_3 = data.TrialHandler(nReps=0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
    seed=None, name='trials_debug_3')
thisExp.addLoop(trials_debug_3)  # add the loop to the experiment
thisTrials_debug_3 = trials_debug_3.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisTrials_debug_3.rgb)
if thisTrials_debug_3 != None:
    for paramName in thisTrials_debug_3:
        exec('{} = thisTrials_debug_3[paramName]'.format(paramName))

for thisTrials_debug_3 in trials_debug_3:
    currentLoop = trials_debug_3
    # abbreviate parameter names if possible (e.g. rgb = thisTrials_debug_3.rgb)
    if thisTrials_debug_3 != None:
        for paramName in thisTrials_debug_3:
            exec('{} = thisTrials_debug_3[paramName]'.format(paramName))
...
# completed 0 repeats of 'trials_debug_3'


# set up handler to look after randomisation of conditions etc
trials_debug_4 = data.TrialHandler(nReps=0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
    seed=None, name='trials_debug_4')
thisExp.addLoop(trials_debug_4)  # add the loop to the experiment
thisTrials_debug_4 = trials_debug_4.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisTrials_debug_4.rgb)
if thisTrials_debug_4 != None:
    for paramName in thisTrials_debug_4:
        exec('{} = thisTrials_debug_4[paramName]'.format(paramName))

for thisTrials_debug_4 in trials_debug_4:
    currentLoop = trials_debug_4
    # abbreviate parameter names if possible (e.g. rgb = thisTrials_debug_4.rgb)
    if thisTrials_debug_4 != None:
        for paramName in thisTrials_debug_4:
            exec('{} = thisTrials_debug_4[paramName]'.format(paramName))
...
# completed 0 repeats of 'trials_debug_4'


# set up handler to look after randomisation of conditions etc
trials_debug_5 = data.TrialHandler(nReps=0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
    seed=None, name='trials_debug_5')
thisExp.addLoop(trials_debug_5)  # add the loop to the experiment
thisTrials_debug_5 = trials_debug_5.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisTrials_debug_5.rgb)
if thisTrials_debug_5 != None:
    for paramName in thisTrials_debug_5:
        exec('{} = thisTrials_debug_5[paramName]'.format(paramName))

for thisTrials_debug_5 in trials_debug_5:
    currentLoop = trials_debug_5
    # abbreviate parameter names if possible (e.g. rgb = thisTrials_debug_5.rgb)
    if thisTrials_debug_5 != None:
        for paramName in thisTrials_debug_5:
            exec('{} = thisTrials_debug_5[paramName]'.format(paramName))
...
# completed 0 repeats of 'trials_debug_5'


# set up handler to look after randomisation of conditions etc
trials_debug_6 = data.TrialHandler(nReps=0, method='random', 
    extraInfo=expInfo, originPath=-1,
    trialList=[None],
    seed=None, name='trials_debug_6')
thisExp.addLoop(trials_debug_6)  # add the loop to the experiment
thisTrials_debug_6 = trials_debug_6.trialList[0]  # so we can initialise stimuli with some values
# abbreviate parameter names if possible (e.g. rgb = thisTrials_debug_6.rgb)
if thisTrials_debug_6 != None:
    for paramName in thisTrials_debug_6:
        exec('{} = thisTrials_debug_6[paramName]'.format(paramName))

for thisTrials_debug_6 in trials_debug_6:
    currentLoop = trials_debug_6
    # abbreviate parameter names if possible (e.g. rgb = thisTrials_debug_6.rgb)
    if thisTrials_debug_6 != None:
        for paramName in thisTrials_debug_6:
            exec('{} = thisTrials_debug_6[paramName]'.format(paramName))
...
# completed 0 repeats of 'trials_debug_6'
