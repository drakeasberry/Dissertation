...
<<global_opts_intuition, echo=TRUE, cache=FALSE>>=
library(knitr)
library(here)
...
intuition_ag <- my_data_long %>% 
  group_by(partNum, group) %>% 
  summarise(logit_corr = mean(logit_corr))
...
intuition_lang_compare <- intuition_ag %>%
  group_by(group) %>% 
  summarise(Mean = mean(logit_corr),
            SD = sd(logit_corr),
            N = n(),
...
intuition_lang_efsize <- cohen.d(intuition_ag$logit_corr, 
        intuition_ag$group, 
        na.rm = TRUE, 
        paired = FALSE)
...
# Create a table for referencing statistics with CIs
intuition_syl_compare <- intuition_syl_ag %>%
  group_by(group, syl_structure) %>% 
  summarise(Mean = mean(logit_corr),
            SD = sd(logit_corr),
            N = n(),
            MOE_95 = qnorm(0.975)*SD/sqrt(N-1),
