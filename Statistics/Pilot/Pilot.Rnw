...
#test <- aov(RTmsec ~ Target*Carrier + Error(participant/(Target*Carrier)), data = my_data)
#summary(test)
...
# CV sylable t.test
t_cv_car <- t.test(cv_car_sum$RTmsec_Mean ~ cv_car_sum$Target, paired = TRUE)#, alternative = "less")
...
# CVC sylable t.test
t_cvc_car <- t.test(cvc_car_sum$RTmsec_Mean ~ cvc_car_sum$Target, paired = TRUE)#, alternative = "less")
...
pilot_stat_compare <- my_data %>% 
  ...
  summarise(Mean = mean(RTmsec),
            SD = sd(RTmsec)) %>% 
  mutate(index = paste0(Target, "_tar-",Carrier,"_car")) %>% 
...
syl_data_summarized <- my_stats(syl_data, 
              grp_col = "participant", 
              sum_col = "Resp_Corr",
              stats = summary_stats)
...
tapply(syl_data$Resp_Corr, syl_data$participant, mean)
@

<<pilot_data_matched, echo = FALSE>>=
my_match_data <- my_data %>% 
  mutate(., Matching = ifelse(Target == Carrier, "Matching", "Mismatching"))

# Run anova   
aov_pilot_matched <- aov_ez("participant", "RTmsec", my_match_data, within = c("Target", "Matching"))
aov_pilot_matched

ag_matched <- aggregate(RTmsec ~ Target + Matching, data = my_match_data, FUN = mean)

#Parallel plots
parallel_example <- ggplot(data=ag_matched,
       aes(x=Target, y=RTmsec, group=Matching, color=Matching)) +
