...
words_to_column <- function(input_tibble,
                            iterator_size,
                            input_vector){
  i <- 1 # r indexing start at 1, not 0
  j <- iterator_size # how many rows need to be created
  ...
  for(item in input_vector){
    ...
    if(i==1){
      ...
      out_vector <- append(out_vector, 
                           as.character(carriers[[item]])) 
    }
    i <- i + iterator_size
    j <- j + iterator_size
  }
...
seg_paths = list.files(path=seg_dir, 
                       pattern = '*.csv', 
                       full.names = TRUE) # list all the files with path
...
for(csv_file in seg_paths){
  ...
  df_participant_modified <- as_tibble(
    select(df_participant_raw, block01:block48))
  
  # subset dataframe to get 'targetSyl' row only once
  df_target_column <- filter(df_participant_modified, 
                             row_number() == 11L)
  ...
  target_syllable_column <- do.call('row_to_column',
                                    list(11,targets))
  # converts block label vector column with appropriate length
  block_column <- do.call('row_to_column',
                          list(11,blocks))
  # grabs all words and puts them into one column
  word_column <- do.call('words_to_column',
                         list(df_participant_raw, 11, blocks))
  
  # Create new dataframe with three new transformed columns
  df_participant_transformed <- add_column(
    df_participant_raw, 
    targetSyl= target_syllable_column,
    block= block_column,
    word= word_column)
  
  # subset to keep only columns necessary
  df_participant_transformed_correct_columns <- select(
    df_participant_transformed,
    fillerCarrier,
    block,
    word,
    targetSyl,
  ...
  df_participant_clean <- filter(
    df_participant_transformed_correct_columns,
    fillerCarrier != 'targetSyl')
  ...
  write_csv(df_participant_clean,
            file.path(transformed_dir, filename))
}


# Segmentation Data Directory
# list all the files with path
seg_files <- list.files(path='analyze_data/transformed/', 
                        pattern = '*.csv',
                        full.names = TRUE) 
...
rm(df_participant_clean, 
   df_participant_transformed_correct_columns,
   df_participant_transformed,
   df_participant_raw,
   df_participant_modified,
   df_target_column,
   targets, blocks,
   target_syllable_column,
   block_column,
   word_column,
   csv_file,
   filename,
...
# list all the files with path
join_paths = list.files(path=join_dir, 
                        pattern = '*[ABCD].csv', 
                        full.names = TRUE)
...
for(csv_condition in join_paths){
  ...
  assign(df_cond_tran, tibble(block = row_tran_1, 
                              word_status = row_tran_2, 
                              target_syl = row_tran_3))
...
all_conditions <- rbind(expCondA.csv,
                        expCondB.csv,
                        expCondC.csv,
                        expCondD.csv)
# Create single table with all 4 condiitons with the 3 transformed columns
all_data_tranformations <- rbind(row_tran_3_expCondA.csv,
                                 row_tran_3_expCondB.csv,
                                 row_tran_3_expCondC.csv,
                                 row_tran_3_expCondD.csv)
...
exp_error <- tibble(word = 'permsio',
                    word_initial_syl = 'CVC') # typo in experimental item added to list
...
df_filler <- rbind.fill(df_rw_filler, 
                        df_pw_filler) %>%
  select(keep_columns)
...
seg_data_join <- df_raw_seg %>%
  ...
  mutate(exp_word_type = ifelse(fillerCarrier == 'carrierItem',
                                'carrier',
                                'filler'),
    target_syl_structure = ifelse(str_length(targetSyl) == 2,
                                  'CV',
                                  'CVC'),
    matching = ifelse(word_initial_syl == target_syl_structure,
                      'matching',
                      'mismatching')) %>%
...
segmentation_data_no_heritage <- subset(
  segmentation_data, 
  segmentation_data$group != "Childhood")

# Create subset of all critical items
seg_critical <- subset(
  segmentation_data_no_heritage, 
  segmentation_data_no_heritage$exp_word_type == 'carrier')
...
seg_critical_correct %>% 
  summarise_at(vars(segRespRTmsec),
               list(quickest = min, slowest = max))
...
seg_critical_misses <- subset(
  seg_critical, 
  seg_critical$segResp == 'None')
...
high_miss_seg_critical_responses <- subset(
  df_seg_critical_errors, 
...
write_csv(
  high_miss_seg_critical_responses,
...
# Prints all responses and frequency of response to filler items
...
seg_filler_responses <- subset(
  seg_filler, 
  seg_filler$segResp == 1)
...
high_seg_filler_responses <- subset(
  df_seg_filler_errors, 
  df_seg_filler_errors$Count >= 43.2) %>%
  rename(Filler_Errors = Count) %>% 
  print()

# Create table of errors committed by high error participants
high_filler_part_raw <- subset(
  seg_filler_responses,
  seg_filler_responses$partNum %in% 
    high_seg_filler_responses$Participant)

# Looks for too quick of response, anything below 200 ms caused by button being held from previous trial
button_held_high <- subset(
  high_filler_part_raw, 
  high_filler_part_raw$segRespRTmsec < 200) %>% 
  select('partNum','segRespRTmsec') #n=121

# Not technical issue filler errors by high error rate participants
button_not_held_high <- subset(
  high_filler_part_raw, 
  high_filler_part_raw$segRespRTmsec >= 200) #n=54

# Number of errors committed by participant after correction for technical limitations
high_filler_part_corrected <- count(
  button_not_held_high, vars = partNum) %>% 
...
if(length(investigate$Participant) == 0){
  ...
  print(sprintf('%d participants removed from data leaving %d participants remaining', 
                length(high_miss_seg_critical_responses$partNum), 
                length(unique(segmentation$partNum))))
  print(sprintf('%s was removed', 
                unique(high_miss_seg_critical_responses$partNum)))
  print(sprintf('%s is eligible', 
                unique(segmentation$partNum)))
  # Write statement for file containing only necessary columns for segmentation analysis
  write_csv(segmentation, 
  ...
  rm(button_held_high, 
     button_not_held_high, 
     high_miss_seg_critical_responses, 
     high_seg_filler_responses, 
     df_seg_critical_errors, 
     df_seg_filler_errors, 
     investigate,
     high_filler_part_raw, 
