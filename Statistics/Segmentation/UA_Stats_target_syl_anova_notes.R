...
tar_syl_main <- emmeans(aov_learners, pairwise ~ target_syl_structure,
                        adjust = 'bonferroni')
# adjustment not needed because it only has one pairwise comparison
tar_syl_main

mat_lex_int <- emmeans(aov_learners, pairwise ~ matching:word_status,
                       adjust = 'bonferroni') 
mat_lex_int <- emmeans(aov_learners, pairwise ~ matching:word_status, adjust = "None") 
# adjustment necessary because it contains 6 pariwise comparisons
mat_lex_int
## Why does Ptukey adjust for 4 estimates and Pbonferroni adjusts for 6?
### Tukey may have optimization happening 
### Recommended plotting before running post-hoc tests
### Recommended running raw (no adjustment for p) then correct yourself for number of 
### tests, but this must be decided beforehand. You cannot change after seeing results
### increases type II error
## Degrees of Freedom emmeans is using pooled error term while t-test is using 
## error term based on one condition given. This pooled error term is adjusting
## the degrees of freedom in emmeans table causing non-interger results
...
# No correction needed for single t-test pairwise comparison
lex_main

#attach(data_lex_ag)
#pairwise.t.test(median_RTlog, word_status, p.adj = 'bonf')
#detach(data_lex_ag)
