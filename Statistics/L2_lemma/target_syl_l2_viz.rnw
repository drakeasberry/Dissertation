...
# Check for outliers
outlier_learner <- online_learners %>% 
  outlier_chk(., online_l2_grouping_stats, "median_RTmsec")

# Check for normality
normality_learner <- online_learners %>% 
...
rm(online_words_learners, 
   online_nonwords_learners,
   online_cv_learners, 
   online_cvc_learners, 
   online_cv_learners_lex, 
   online_cvc_learners_lex)

# Remove checks and unused plots
rm(online_l2_tarsyl_mat_int, 
   online_l2_lex_tarsyl_int, 
   normality_learner, 
   outlier_learner)
