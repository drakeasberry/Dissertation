...
Previous studies that investigated speech processing used sample sizes between 20 and 44 participants \parencite{Bradley1993-qq,Chetail2014-nm,Sebastian-Galles1992-xd,Segui1981-uf}.%\ttodo{Please provide specific references to these previous studies.} 
...
In order to collect the data that has been reported throughout this dissertation, several specialized instruments were utilized. A button box was used to capture reaction times of participant responses (Section \ref{ssec-button-box}). %\ttodo{au: This is not a grammatical sentence.} 
The second was the use of open source software, PsychoPy and PsychoJS, to build the experiments, which has been widely used in previous linguistic and behavioral science studies (Section \ref{ssec-psychopy}). %\ttodo{au: Be specific - which software?} 
...
Given the difference in average age,\ttodo{I wonder if age might have other effects. I suppose this has happened to everybody doing experiments the last year or so, switching to online recruitment and having older subjects.} it is not surprising that the average length of time from the onset of Spanish acquisition was also higher in the online group, 
...
%The data collection locations for the entire dissertation included only the United States and Mexico. Throughout the project, data collection methods changed from laboratory conditions for in-person data collection to online data collection methods as a necessity in response to the COVID-19 social distancing requirements. Below are detailed descriptions of both data collection methods and site locations. 
