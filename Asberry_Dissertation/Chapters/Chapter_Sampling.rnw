...
However, language acquisition age would be a much more likely variable to influence the results of this study as it revolves around phonological acquisition processes rather than acquiring new lexical items. As such, a Welch two-sample t-test was used to look at the difference in average age of acquisition for Spanish. The age of Spanish acquisition in the online group, M=\Sexpr{round(eng_stat_compare["Online", "Mean_esp_age"],2)}, SD=\Sexpr{round(eng_stat_compare["Online", "SD_esp_age"],2)} at 95\% CI [\Sexpr{round(t_eng_online_esp_age$conf.int[1],3)}, \Sexpr{round(t_eng_online_esp_age$conf.int[2],3)}], was not significantly different than the L2 Spanish group recruited in-person, M=\Sexpr{round(eng_stat_compare["In-person", "Mean_esp_age"],2)}, SD=\Sexpr{round(eng_stat_compare["In-person", "SD_esp_age"],2)} [\Sexpr{round(t_eng_inperson_esp_age$conf.int[1],3)}, \Sexpr{round(t_eng_inperson_esp_age$conf.int[2],3)}]. The mean difference, M=\Sexpr{round(abs(eng_stat_compare["Online", "Mean_esp_age"] - eng_stat_compare["In-person", "Mean_esp_age"]),2)}, revealed the differences in age of Spanish acquisition between groups failed to reach significance, \emph{t}(\Sexpr{round(eng_esp_age_compare$parameter,2)})= \Sexpr{round(eng_esp_age_compare$statistic,4)}, p < \Sexpr{round(eng_esp_age_compare$p.value,4)} [\Sexpr{round(eng_esp_age_compare$conf.int[1],3)}, \Sexpr{round(eng_esp_age_compare$conf.int[2],3)}].
