...
p < \Sexpr{round(t_online_l2_cv_mat_int$p.value,4)}. 
...
In the CVC target dataset, it revealed that the L2 Spanish group detected CVC syllables significantly slower for the matching condition, 
...
The effect size of matching condition in the word dataset was 
...
Figure \ref{plt-online-l2-tar-mat-s-int} plots the estimated marginal means for the interaction between \emph{Target} and \emph{Matching} for L2 learners of Spanish. It shows that this interaction was driven by the fact that participants responded significantly faster to the CV targets in the matching condition and CVC target in the mismatching condition.
...
participants. Both datasets, words and nonwords, were then individually submitted to a one-tailed t-test. This revealed that the L2 Spanish group did not detect CV syllables faster in nonwords, 
...
In the real word dataset, it also revealed that the L2 Spanish group did not detect CV syllables significantly faster in nonwords, 
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In order to break down the final trending interaction between \emph{Matching} and \emph{Lexicality} factors, a post-hoc analysis was run. The L2 Spanish dataset was was further subset into two dataframes: one for real words and one for nonwords. Each dataset consisted of 
...
\section{Discussion}
This chapter reported on the \Sexpr{length(unique(my_l2_data$partNum)) + length(unique(my_l1_data$partNum))} participants who completed an online visual Spanish letter sequence monitoring experiment. The experiment run in this chapter attempted to replicate the findings of previous research in terms of Spanish letter sequence monitoring abilities as an indicator of the use of a syllabic segmentation strategy. The current chapter utilized \Sexpr{length(unique(my_l1_data$partNum))} native Spanish speaker and \Sexpr{length(unique(my_l2_data$partNum))} native English speakers who were L2 learners of Spanish as participants. Crucially, it also attempted to investigate and determine why a null effect was found in chapter \ref{ch-seg-lab}. It continued to investigate the use of the letter sequence monitoring experimental design in order to address whether or not utilizing a syllabic segmentation strategy when processing Spanish was learnable. 
...
The outcome of the native Spanish speaker control group in the current chapter replicated the results of \textcite{Bradley1993-qq} experiment 1, which also utilized native Spanish monolinguals. In line with my hypoothesis, the L1 Spanish group results supported the use of the syllabic segmentation strategy when processing Spanish. Given that the L1 Spanish showed sensitivity to the syllable during language segmentation, this finding gives additional support to the null effect found in chapter \ref{ch-seg-lab} and the previous Spanish--English bilingual results \parencite{Bradley1993-qq}.
This eliminated the Spanish experimental items selection and the completely visual design factors as the root cause for the null findings in chapter \ref{ch-seg-lab} of the current project because these items these items did support a syllabic-based segmentation strategy. Moreover, this finding also supported the viability of conducting a sufficiently powered phonological study where reaction time was crucial to the interpretation of the results in an online environment without a specialized button response box. 

Given the background that L1 Spanish speakers showed evidence of the syllabic segmentation effect, attention is turned to L2 learners of Spanish---the target group in the current study. Previous literature has suggested that English speakers are unable to employ syllabic segmentation strategies even under optimal conditions where monitored languages had been shown to support the strategy \parencite{Bradley1993-qq,Cutler1986-zl,Cutler1992-qq}. The findings of chapter \ref{ch-seg-lab} also supported this claim and served as the basis for the current hypothesis that the L1 English participants would not exhibit evidence of the syllable-based segmentation strategy. Therefore, it was quite surprising that a strong trend was found for the L2 learners of Spanish suggesting some sensitivity to the syllable during Spanish word segmentation. This sensitivity to the matching condition was driven the nonwords utilized in the study where initial syllables of nonwords matching the syllable structure of the target were detected faster than nonwords mismatching the target syllable structure. 

In chapter \ref{ch-seg-lab} participants showed a preference of monitoring for CVC target sequences, but this was not true of the findings in the current experiment. In the current chapter, no reaction time advantage was revealed for either target structure---CV or CVC. However, there were interactions between target structure and matching conditions as well as target structure and lexicality conditions. \ttodo{expand on these findings or interactions}


One possible explanation for these findings is it that participants were not processing the words and nonwords in the same manner. In other words, they could have been making decisions prelexically for nonwords and postlexically for words. \ttodo{this will need to be addressed somewhere}


\textcite{Bradley1993-qq} reported that written targets produced slower repsonses than spoken targets---618 ms versus 583 ms. \ttodo{page 209} They argued that the spoken target presentation limited variability and aided the participants in the segmentation task. The results here replicated their findings with an average of 769 ms for words. Only the average for words can be compared here because \textcite{Bradley1993-qq} did not include words their study, but the average response time for nonwords in the current study was \ttodo{insert RT nonwords average} ms. Indeed, this is in line with slower reaction times for visual targets. The fact the visual target and stimuli presentation may have accounted for longer reaction times. On the other hand, it could be related to online method of experimentation and lack of hardware control contributing to longer reaction times.

\textcite{Bradley1993-qq} argued that the prelexical claim in French given by \textcite{Mehler1981-vi} was not upheld in Spanish. The use of nonword condition in the current study should allow this same claim to be made within the same speaker group. Zwitserlood 1991 studied Dutch and showed the crossover effect followed the signal where the average response time was around 600 ms. This was used as evidence that slow response times do not necessarily rule out prelexical decisions or codes. \ttodo{check bradley article to reference correctly as cited in method}

In Spanish monitoring experiments, reaction times have been shown to be a factor contributing to finding evidence of syllabic segmentation strategy. Very fast reaction times have been argued to mask the effect while slowing participant responses have been argued to cause the effect to remerge. \textcite{Sebastian-Galles1992-xd}, increased their average reaction times for monitoring in Spanish from 355-394 ms where they found no effect to 586-643 ms by adding a semantic relationship task to their design and the syllabic effect reemerged. Similarly, \textcite{Bradley1993-qq} found a syllabic effect in their Spanish monolingual participants with reaction times ranging from 556-609 ms, but not with their bilingual speakers who had reaction times ranging from 469-524 ms. In chapter \ref{ch-seg-lab} of this dissertation, L1 Spanish--L2 English speakers ranged from 528-544 ms and L1 English--L2 Spanish speakers ranged from 523-545 ms for real words. The previous results of this dissertation also did not find evidence of a syllabic segmentation strategy. The current chapter saw reaction times ranging from - ms for L1 Spanish and from - ms for L1 English speakers for real words. The longer reaction times could have been from hardware setups and running experiment online. \ttodo{this should provide support against argurment that repetition priming was at play. Can I run this type of analysis?} 
...
Power sample size was met with 44 participants from Spanish and 55 from English. \ttodo{insert calls instead of hard numbers}
...
As in chapter \ref{ch-seg-lab}, the use of visual word recognition paradigm to study segmentation strategies remained a major design difference because it assumes the visual input is activating phonological representations. The advantage of using a completely visual design is that it eliminates the possibility of phonetic detail affecting results, which had been a potential confound in previous auditory studies. Chapter \ref{ch-seg-lab} replicated a null effect of a Spanish--English bilingual auditory monitoring experiment from \textcite{Bradley1993-qq} with a visual monitoring experiment. In the current study, a second auditory monitoring experiment using Spanish monolinguals in \textcite{Bradley1993-qq} was succesfully replicated. In both cases, the monolingual Spanish speakers did show evidence of a syllabic segmentation strategy. Given that in both cases the monolingual groups were tested with the same experimental items as the bilingual groups, it gives stronger support for two claims. The first is that the acquisition of English is somehow affecting the way in which Spanish native speakers approach word segmentation in Spanish. While this study is not able to tease apart these differences, it is not unlike previous findings for romance langauges such as French where monolinguals and bilinguals differed \parencite{Cutler1992-qq}. The second claim is in support of using a visual experimental design to study phonological processes utilized in segmentation strategies is a viable option. These replication studies suggest that representations of syllable are being activated at some level of processing during the visual letter sequence monitoring. This falls in line with other previous research that has suggested phonological activation is occurring when a person reads.
