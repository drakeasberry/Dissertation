...
\section{Results}
These results indicated that it was possible to see evidence of the syllabic segmentation strategy in a L1 Spanish monolingual group. It also showed evidence of L1 English--L2 Spanish bilingual group being able to learn this strategy. In addition, these results provided evidence that this syllabic segmentation effect could be captured through a completely visual word segmentation design.
...
The log transformed reaction times for the \Sexpr{length(unique(online_natives$partNum))} L1 Spanish controls were submitted to a (2) x (2) x (2) repeated measures ANOVA with \emph{Lexicality, Target} and \emph{Matching} as factors. No statistically significant interactions were found as p > 0.05 in all cases. The ANOVA revealed a significant main effect of \emph{Matching} \emph{F}(
...
The main effect of matching conditions was of a 
...
participants. Both CV and CVC target datasets were then each submitted to a one-tailed t-test. This revealed that the L1 English group detected CV syllables faster in the matching condition, 
...
p < 0.0001 %\Sexpr{round(t_online_l2_cv_mat_int$p.value,4)}. \ttodo{check p value in stats file}
...
%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%%
In order to break down the interaction between the \emph{Target} and \emph{Lexicality} factors, another post-hoc analysis was run. The L1 English dataset was split into two dataframes: one for real words and one for nonwords. Each dataset consisted of 
...
revealed a non-statistically significant difference with regards to the target syllable structure in nonwords, 
...
@      
\caption[Estimated Marginal Means: Online Experiment Target Group]{L1 English log transformed reaction times reported as a factor of target syllable structure and lexicality conditions.}
...
This chapter concludes with results that support a syllable-based segmentation strategy for Spanish. It also suggests that this strategy may be learnable by L1 English speakers who are second language learners of Spanish. Furthermore, it opens up the door future studies to utilize a visual based methodology to study phonological processes. Lastly, it suggests that investigating these segmentation strategies and/or other reaction time based studies are becoming increasingly viable through online experimental platforms.