...
Chapter \ref{ch-seg-online} included monolingual speakers and the BLP is not built for people who only speak one language. Therefore, a modified version of the BLP demographic questions were retained and included at the experimental start up screen. Participants typed or choose from prepopulated drop-downs to respond each field prior to starting the the experiment.

\subsection*{L1 Monoligual Spanish Speakers}
\begin{singlespacing}
\begin{enumerate}
\item Edad\newline \emph{Age}
\item Sexo\newline \emph{Sex}
